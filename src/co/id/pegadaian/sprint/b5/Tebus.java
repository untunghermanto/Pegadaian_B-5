	...
	public static void tebus() {
		...
		for (Data value : dataArrayList) {
			if (!value.getStatus().equalsIgnoreCase("Lunas")) {
				System.out.println(String.format(formatter, value.getId(), value.getProductCategory(), value.getPrice(),
						value.getStatus(), value.getUtang()));
			}
		}
		System.out.println("+------+-----------------+------------+---------------------------+-----------------+");
			...
			try {
				...
				if ((utang % 10000 == 0)) {
					for (int i = 0; i < dataArrayList.size(); i++) {
						if (dataArrayList.get(i).getId() == id) {
							...
							if (sisa > 0) {
								...
								System.out.println(
										"Pembayaran anda melebihi sisa pinjaman anda. Sisa pinjaman anda adalah "
												+ dataArrayList.get(i).getUtang());
								flag = 1;
							}
						}
					}
					if (flag == 0) {
						...
						System.out.println("Pembayaran harus kelipatan 10000");
					}
			} catch (Exception e) {
				System.out.println("Inputan yang anda masukkan salah");
				inputMenu.next();
